...
class VaspBandSetup:
    ...
    def generate_incar(self):
        ...
        # 优先级: [band_params] > [incar_params] > 默认模板
        if "incar_params" in self.config:
            final_params.update(self.config["incar_params"])
        if "band_params" in self.config:
            final_params.update(self.config["band_params"])
        
        print(f"正在生成 INCAR (Band)...")
    ...
    def generate_kpoints(self):
        """使用 vaspkit 生成 KPOINTS (K-Path)"""
        print("正在调用 vaspkit 生成能带路径 (303)...")
        try:
            # 在 eband 目录下运行 vaspkit
            ...
            stdout, stderr = process.communicate(input="303\n")
            
            kpath_file = os.path.join(self.work_dir, "KPATH.in")
            kpoints_file = os.path.join(self.work_dir, "KPOINTS")
            
            if os.path.exists(kpath_file):
                shutil.copy2(kpath_file, kpoints_file)
                print(f"KPOINTS 已从 KPATH.in 拷贝完成。")
            else:
                print(f"错误: vaspkit 未生成 KPATH.in。")
    ...
    def run(self, run_calc=False):
        try:
            ...
            print(f"\nBand 计算文件已在 {self.work_dir} 目录中准备就绪！")
...
def main():
    parser = argparse.ArgumentParser(description="VASP Band Setup Script")
    ...
    setup = VaspBandSetup(config_file=args.config, struct_file=args.input)
