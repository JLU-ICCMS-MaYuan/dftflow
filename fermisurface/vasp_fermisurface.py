...
class VaspFermiSetup:
    def __init__(self, config_file="input.toml", struct_file=None):
        ...
        # 2. 默认 INCAR 模板 (针对 Fermi Surface 优化)
        self.incar_template = {
            "SYSTEM" : "VASP_FermiSurface",
            "ISTART" : 1,
            "ICHARG" : 11,
            "PREC"   : "Accurate",
            "IBRION" : -1,
    ...
    def generate_incar(self):
        ...
        # 优先级: [fermi_params] > [incar_params] > 默认模板
        if "incar_params" in self.config:
            final_params.update(self.config["incar_params"])
        if "fermi_params" in self.config:
            final_params.update(self.config["fermi_params"])
        
        print(f"正在生成 INCAR (FermiSurface)...")
    ...
    def run(self, run_calc=False):
        try:
            ...
            print(f"\nFermi Surface 计算文件已在 {self.work_dir} 目录中准备就绪！")
