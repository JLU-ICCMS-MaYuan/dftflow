...
class VaspLocpotSetup:
    ...
    def generate_incar(self):
        ...
        # 优先级: [locpot_params] > [incar_params] > 默认模板
        if "incar_params" in self.config:
            final_params.update(self.config["incar_params"])
        if "locpot_params" in self.config:
            final_params.update(self.config["locpot_params"])

        print("正在生成 INCAR (LOCPOT)...")
    ...
    def run(self, run_calc=False):
        try:
            ...
            print(f"\nLOCPOT 计算文件已在 {self.work_dir} 目录中准备就绪！")
...
def main():
    parser = argparse.ArgumentParser(description="VASP LOCPOT Setup Script")
    ...
    setup = VaspLocpotSetup(config_file=args.config, struct_file=args.input)
